...
def ler_com_kokoro(texto, pipeline):
    with st.spinner('🎧 Gerando áudio com Kokoro, aguarde...'):
        ...
        audio_total = []
        for parte in partes[1:]: # O index 0 vem o nome do relatorio e nao seçoes
            if not parte.startswith("Relatório"):
...
if option == 'Profile':
    ...
    if 'markdown' in st.session_state and uploaded_file is not None:
